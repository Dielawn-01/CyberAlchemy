...
stem_check_v3.py
================
Unified V3 STEM Review Engine for the InfoPhys/Protoreal Corpus.
...
Usage:
    python3 stem_check_v3.py [--verbose]
"""
...
def section(name):
    ...
    print(f"\n{'═'*70}")
    print(f"  V3 MATH/PHYSICS TIER: {name}")
    print(f"{'═'*70}")
    ...
    if condition:
        ...
        if is_warning:
            ...
            if schwarzian_torque >= UPSILON_LIMIT:
                ...
                print(f"{'═'*70}")
                print(f"  The V3 engine breached the maximum allowable friction (Υ = {schwarzian_torque:.1f}).")
...
def run_audits():
    ...
    bridge_files = {
        ...
        "Ch.11": ("11_Archetypal_Incentive_Theory.tex", "Five-Coordinate Condensation"),
        "Ch.12": ("12_Triple_Helix_Mechanics.tex", "171-Frame Metabolic Lifecycle"),
        "Ch.13": ("13_Bionetic_Ambrosia_Protocol.tex", "Biological Chip Architecture"),
        "Ch.14": ("14_Procedural_Game_Design.tex", "Semantic Subspaces"),
    }
    ...
    # ------------------------------------------------------------
    # Tier 6: Volume Structure Validation
    # ------------------------------------------------------------
    section("§6: Volume Structure Validation")
    ...
    if vol1_path.exists():
        ...
        check("Vol 1 includes Part I (Logical Creativity)", "Logical Creativity" in vol1)
        check("Vol 1 does NOT include Part II", "Informational Creativity" not in vol1,
              "Part II should be in Vol 2")
    else:
        check("Vol 1 Master exists", False)

    if vol2_path.exists():
        vol2 = vol2_path.read_text(encoding="utf-8", errors="replace")
        check("Vol 2 title: Informational and Physical", "Informational and Physical" in vol2)
        check("Vol 2 includes Part II (Informational)", "Informational Creativity" in vol2)
        check("Vol 2 includes Part III (Physical)", "Physical Creativity" in vol2)
        check("Vol 2 includes Epilogue", "Epilogue" in vol2)
    else:
        check("Vol 2 Master exists", False)

    # Verify PDFs exist
    check("Vol 1 PDF compiled", (ROOT_PAPERS_DIR / "02_Vol1_Master.pdf").exists())
    check("Vol 2 PDF compiled", (ROOT_PAPERS_DIR / "03_Vol2_Master.pdf").exists())
...
def main():
    ...
    print(f"\n{'═'*70}")
    print(f"  V3 ENGINE HALTING STATE REPORT")
    print(f"{'═'*70}")
