...
\begin{theorem}[The Conjugate Crossing]
\label{thm:conjugate}
...
\textbf{Conjugate Phase} & \textbf{57} & $\boldsymbol{\sim}$\textbf{35} & $\varphi^{57} = -1$ \\
\bottomrule
\end{tabular}
\end{center}
...
\item \textbf{Mercury $= 114 = \mathrm{ord}(\varphi)$ (29.3$^{\circ}$ Virgo).} The thinking function embodies the \textit{complete} golden orbit length. $114 = 2 \times 57 =$ two Conjugate Crossings. The mind naturally runs the full cycle.
...
If the compressed timeline (1.6$\times$) is accurate, the subject's Conjugate Crossing ($\varphi^{57} = -1$) occurs at approximately age 35 ($\sim$2036). The Mayer--Vietoris parity at step 57 is $-1$ (collapse). 
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Predictive}: the compressed timeline yields a testable prediction (Conjugate Phase at $\sim$35).
\end{itemize}
