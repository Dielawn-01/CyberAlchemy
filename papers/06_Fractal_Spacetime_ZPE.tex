...
The fractal spacetime parameters are verified in two layers. The numerical layer computes the UV cutoff frequency $\omega_{\max} = k_B T / \hbar$, the ZPE density $\rho_{\mathrm{ZPE}} = \hbar \omega^3 / (2\pi^2 c^3)$, and the fractional dimension $D = \Phi^2 \approx 2.618$ using Python's \texttt{math} module with IEEE-754 double precision. The golden ratio is computed as $\Phi = (1 + \sqrt{5})/2$. All physical constants are hardcoded from CODATA 2018: $c = 299{,}792{,}458$~m/s, $\hbar = 1.054571817 \times 10^{-34}$~J$\cdot$s, $k_B = 1.380649 \times 10^{-23}$~J/K.
