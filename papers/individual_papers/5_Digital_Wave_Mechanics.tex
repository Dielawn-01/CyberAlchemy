...
The subgroup $\langle \omega \rangle = \{1, 94, 134\} \subset \FF_{229}^\times$ is isomorphic to $Z(\mathrm{SU}(3)) = \ZZ/3\ZZ$, and the confinement identity $1 + \omega + \omega^2 \equiv 0 \pmod{229}$ is the finite-field avatar of the QCD singlet condition.
\end{theorem}
...
\end{align}
Arc rotation by $\omega = 94$ (the cube root of unity) maps one label to the next. This reproduces the count and cyclic transition structure, not the measured PMNS matrix. A PMNS-level model would require three mixing angles, a CP phase, mass splittings, and an oscillation Hamiltonian.
