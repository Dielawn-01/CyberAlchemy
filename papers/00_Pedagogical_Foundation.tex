...
\paragraph{The database parallel.}
If you have ever seen a spreadsheet, you already have a working mental model
of a \textbf{relational database}. A database table is a set of rows; each row
is an element. The SQL language gives us the same operations we just learned,
in a syntax designed for data:
\begin{center}
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Set Theory} & \textbf{SQL (Postgres)} \\
\midrule
...
\noindent This is not a metaphor. The mathematical foundations of relational
databases \emph{are} set theory, formalized by Edgar Codd in 1970. When you
write SQL, you are writing set theory in a slightly different alphabet. The
companion module \texttt{principia.foundations.sets} implements a
...
of mutual relations} --- what we will later call the semantic condensation
invariant.
...
\emph{relation}. Adding a \texttt{UNIQUE} constraint on the joining column
enforces injectivity.

\paragraph{The Python parallel.}
Functions in Python are, well, functions. But the deeper connection is the
\texttt{dict} (dictionary): a mapping from keys to values that enforces the
function property --- each key maps to exactly one value.

\begin{verbatim}
# A function from names to ages
f = {'Alice': 30, 'Bob': 25, 'Carol': 28}
print(f['Alice'])  # 30 --- exactly one output per input
\end{verbatim}
...
first step in his construction of the set of transfinite ordinals. I don't
care what the hell they identify as; I won't extend our treatment to them
here. In all seriousness, $\mathbb{N}$ is all we need for right now.
